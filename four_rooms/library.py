...
def epsilon_greedy_policy_improvement(env, Q, epsilon=1):
    ...
    def policy_improved(state, epsilon=epsilon):
        probs = np.ones(env.action_space.n, dtype=float) * (
...
def epsilon_greedy_generalised_policy_improvement(env, Q, epsilon=1):
    ...
    def policy_improved(state, goal=None, epsilon=epsilon):
        ...
        values = (
            [Q[state][goal]] if goal else [Q[state][goal] for goal in Q[state].keys()]
        )
        if len(values) == 0:
...
#########################################################################################
def Q_learning(
    env, Q_optimal=None, gamma=1, epsilon=1, alpha=1, maxiter=100, maxstep=100
):
    """
...
def Goal_Oriented_Q_learning(
    ...
    behaviour_policy = epsilon_greedy_generalised_policy_improvement(
        env, Q, epsilon=epsilon
    )
